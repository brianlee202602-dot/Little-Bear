...
本文基于当前 `design_docs_history/` 下的设计文档整理 Little Bear 的设计思路、核心流程、模块关系和落地顺序，作为正式编码、评审、联调和验收时的总览入口。

相关依据：

- `design_docs_history/README.md`
- `design_docs_history/架构设计文档.md`
- `design_docs_history/MVP范围说明.md`
- `design_docs_history/项目执行流程图.md`
...
- `design_docs_history/development/本地开发环境.md`
- `design_docs_history/planning/正式编码前任务追踪.md`
...
- 本地开发和 P0 测试使用外部 embedding、rerank、LLM 服务或企业模型代理跑通，不再依赖内置模型模拟服务。
...
当前不急于拆微服务。后续如果 Query、Import、Permission、Config 或 Model Provider Adapter 成为独立热点，再按模块边界拆出服务。

### 2.3 启动配置最小化，业务配置全部进入 active config

API、Worker 和模型 Provider Adapter 进程启动时只允许读取数据库连接配置和进程参数。
...
这些业务配置必须通过初始化接口写入数据库，并由 Config Service 读取 `active_config v1`。ServiceBootstrap 再基于 active config 初始化 Redis、Secret Store、Object Storage、Vector Store、Keyword Search、模型 Provider Adapter、Audit Sink 和 Rate Limiter。
...
### 2.6 模型能力通过外部 Provider Adapter 访问

业务模块不直接调用 vLLM、TEI 或第三方模型 SDK，而是通过统一 adapter 层访问外部 provider。

所有模型能力通过外部 provider adapter 访问：
...
P0 直接配置 TEI embedding、TEI rerank 和 OpenAI-compatible LLM provider。这样可以在接口、日志、降级和测试阶段就暴露真实模型服务的延迟、超时、维度和成本问题。
...
- P0 只做初始化、认证、组织权限、知识库文档、导入索引、权限过滤查询、citation、审计和外部模型 provider 验收。
...
- `design_docs_history/modules/08-查询服务设计实现文档.md`
- `design_docs_history/modules/09-检索上下文与答案生成设计实现文档.md`
- `design_docs_history/modules/10-模型网关设计实现文档.md`
- `design_docs_history/modules/11-审计与可观测性设计实现文档.md`
- `design_docs_history/modules/12-接口网关与高并发设计实现文档.md`
- `design_docs_history/modules/13-部署与运维设计实现文档.md`
...
- `design_docs_history/development/本地开发环境.md`
- `design_docs_history/testing/测试计划.md`
- `design_docs_history/operations/部署与发布检查清单.md`

设计动作：

- 把初始化、认证、导入、索引、查询、权限变更、删除阻断、配置发布和审计串成可复现流程。
- 本地使用 Docker Compose 启动 PostgreSQL、Redis、MinIO、Qdrant、TEI embedding 和 TEI rerank。
- P0 验收需要可控外部 LLM provider 或企业模型代理。
...
8. ServiceBootstrap 基于 active config 初始化 Redis、Secret Store、MinIO、Qdrant、Keyword Search、模型 Provider Adapter、Audit Sink、Rate Limiter。
...
| Model Provider Adapter | Embedding、Rerank、LLM 的统一访问层 | 模型响应、模型调用日志 |
...
本地基础设施由 `docker-compose.yml` 管理，`Makefile` 封装基础设施生命周期、API 主程序和前端开发入口：
...
1. make env
2. make up
3. 检查 PostgreSQL、Redis、MinIO、Qdrant、TEI embedding、TEI rerank 和 LLM provider 是否健康
...
15. 无权限用户无法召回部门文档。
16. 验证权限收紧、文档删除、缓存隔离和外部模型 provider 降级。
...
| 外部模型服务不稳定 | 开发测试受下载、延迟、成本和供应商限流影响 | 固定 provider profile，设置超时、降级、调用日志和独立模型服务健康检查 |
...
- 不允许为了方便绕过 Config Service、Permission Service 或模型 Provider Adapter。
