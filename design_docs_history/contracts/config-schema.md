...
机器可校验 JSON Schema 已落地到 `design_docs_history/contracts/config.schema.json`。本文负责说明字段语义、校验意图和运行约束；实现中的 schema 校验应以 `design_docs_history/contracts/config.schema.json` 为准。
