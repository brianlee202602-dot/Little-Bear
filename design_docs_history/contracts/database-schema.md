...
- `design_docs_history/contracts/config.schema.json`
...
用途：保存配置版本下的配置项。P0 使用 global scope 和 `design_docs_history/contracts/config.schema.json` 的顶层配置 key。
...
- `secret_ref` 必须匹配 `design_docs_history/contracts/config.schema.json` 中 SecretRef 格式。
