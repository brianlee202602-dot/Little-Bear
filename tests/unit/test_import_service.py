...
def test_create_document_import_rejects_permission_outside_parent_kb() -> None:
    ...
    session.results = [
        ...
        _Result(one_or_none=_Row({"department_id": target_department_id, "status": "active"})),
    ]
    ...
    assert exc_info.value.error_code == "IMPORT_DOCUMENT_PERMISSION_OUTSIDE_KB_SCOPE"
...
def test_advance_permission_refresh_job_refreshes_payload_and_skips_import_stages(
    ...
    class _FakeIndexingService:
        ...
        def refresh_permission_payloads(self, _session, *, request_json):
            self.requests.append(request_json)
            return {"chunk_count": 1, "vector_payload_count": 1}
    ...
    session.results = [
        ...
        _Result(one=_job_row(status="running", stage="publish")),
    ]
    service = ImportService()
    monkeypatch.setattr(
