...
本文说明 Little Bear 交付版系统架构。它只说明系统组成、职责边界和依赖方向，不重复接口字段和具体代码实现。
...
| `Redis` | 作为基础设施依赖参与 bootstrap；可承载缓存、锁、通知等派生运行能力 | 交付版不包含最终答案缓存业务闭环 |
...
Redis 作为基础设施依赖被纳入 bootstrap 检查。最终答案缓存、分布式限流、配置通知等业务缓存能力不属于交付版运行闭环。
