...
1. [架构设计文档](架构设计文档.md)
2. [公共实现约束](modules/00-公共实现约束.md)
3. [核心数据模型](modules/14-核心数据模型设计实现文档.md)
4. 按模块阅读 `docs/modules/` 下的实现设计文档
...
| [03-组织服务设计实现文档.md](modules/03-组织服务设计实现文档.md) | 企业、部门、岗位、项目组、用户组织关系 |
| [04-权限服务设计实现文档.md](modules/04-权限服务设计实现文档.md) | RBAC/ABAC/ACL、权限上下文、权限过滤下推 |
...
- API 服务只通过环境变量读取启动配置，业务配置全部来自初始化接口发布后的 active config。
- 未初始化时只开放 setup 状态和初始化接口，普通业务 API 必须返回 `SETUP_REQUIRED`。
- 权限过滤必须下推到向量检索和关键词检索，禁止全量召回后再做应用层过滤。
- 查询候选进入上下文前仍必须执行元数据可见性、access block 和引用有效性 gate，作为索引权限字段的兜底校验。
